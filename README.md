...
[示例网站](https://blog.aoidayo.site/hugo-encrypt-showcase/)
...
## 已知问题
- **文章目录重建**：整篇/局部加密后不重新渲染目录。目录由主题正常生成，插件不做主题适配。
- **搜索索引保护**：会自动清理 `public/index.json` 中受保护文章的 `content`、`summary`、`description` 字段，但其他搜索索引文件需要按主题额外适配。
- **RSS 防泄露**：需要提供 RSS 模板适配方式，避免 `.Summary` 把加密内容输出到 feed。
...
- 如果主题会把 `.Summary`、`.Plain`、`.Content` 输出到搜索或 RSS：需要额外适配，避免泄露。
